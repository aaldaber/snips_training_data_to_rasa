        ...
        for eachvalue in eachv:
            ...
            for oneitem in eachvalue['data']:
                ...
                if oneitem.get('entity'):
                    ...
                    entities.append({'start': running_length, 'end': running_length + len(oneitem['text']), 'value': oneitem['text'], 'entity': oneitem['entity']})
                running_length += len(oneitem['text'])

            rasa_data['rasa_nlu_data']['common_examples'].append({"text": sentence, 'intent': eachkey, 'entities': entities})
            #print(sentence)
...
with open('new_data_full.json', 'w', encoding='utf-8') as f:
